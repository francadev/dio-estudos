...
def validate_numero_telefone(phone_number):
    ...
    pattern = re.compile("^\(\d{2}\)\s9\d{4}-\d{4}$")
    # A função 're.match()' para verifica se o padrão definido corresponde ao número de telefone fornecido.
    # O 're.match()' retorna um objeto 'match' se houver correspondência no início da string, caso contrário, retorna
    # None'. (XX) 9XXXX-XXXX onde X representa um dígito de 0 a 9.
...
# TODO: ...resultado retornado na variável 'result'.
